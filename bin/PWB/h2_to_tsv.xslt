...
/>
  ...
  <xsl:param name="outputdir">None</xsl:param>
  ...
  <xsl:template match="/">
    <xsl:text>WbConnect -url=</xsl:text>
  ...
  <xsl:template match="table-def">
    ...
    <xsl:text>WbExport -type=text</xsl:text>
    <xsl:text> -file="</xsl:text>
    <xsl:value-of select="$outputdir"/>   
    <xsl:text>/</xsl:text>
    <xsl:value-of select="translate(table-name, $uppercase, $lowercase)" />  
    <xsl:text>.tsv" -continueOnError=false -encoding=UTF8 -header=true -decimal='.' -maxDigits=0 -lineEnding=lf -clobAsFile=false -blobType=base64 -delimiter=\t -replaceExpression='(\n|\r\n|\r|\t|^$)' -replaceWith=' ' -nullString=' ' -showProgress=10000; SELECT</xsl:text>
