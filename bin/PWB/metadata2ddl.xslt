...
<!-- TODO: Test å fjerne "not null" fra generert ddl -->
<xsl:output
  encoding="UTF-8"
  method="text"
  indent="no"
...
-->
  ...
  <xsl:param name="quoteAllNames">false</xsl:param>
  <xsl:param name="commitAfterEachTable">false</xsl:param>
  <xsl:param name="unrestrictedVarchar">false</xsl:param>
  <xsl:param name="identifierCleanup">none</xsl:param>
  ...
  <xsl:param name="includeIndexes">false</xsl:param>
...
<!-- 
  ...
  <xsl:template match="table-def">
    ...
    <xsl:if test="$pkcount &gt; 0">
      ...
      <xsl:text>)</xsl:text>
      <xsl:call-template name="process-fk">
        <xsl:with-param name="tblname" select="$tablename"/>
      </xsl:call-template>   
      <xsl:value-of select="$newline"/>
    </xsl:if>
    <!-- <xsl:value-of select="$newline"/> -->
        ...
        <xsl:if test="$tblname = $table">
          ...
          <!-- <xsl:for-each select="foreign-keys/foreign-key[disposed !='true']"> -->
            ...
            <xsl:for-each select="source-columns/column">
              ...
              <!-- <xsl:sort order="ascending" select="text()"/> <!-\- TODO: Fix for mysql -> lage samme for pk nødvendig? -\-> -->
              <xsl:call-template name="write-object-name">
  ...
  <xsl:template name="write-data-type">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$type-id = -1"> <!-- LONGVARCHAR -->
        ...
        <!-- <xsl:text>varchar(4000)</xsl:text> -->
      </xsl:when>
      <xsl:when test="$type-id = -16"> <!-- NTEXT -->
        <xsl:text>text</xsl:text>
        <!-- <xsl:text>varchar(4000)</xsl:text> -->
      </xsl:when>
        ...
        <xsl:if test="$scale = 0 or $scale = ''">
          ...
          <xsl:if test="$precision &gt; 10">
            <xsl:choose>
		      ...
		      <xsl:otherwise>
	        	<xsl:text>decimal</xsl:text><!-- TODO: Endret fra bigint for test -->
	          </xsl:otherwise>
	        </xsl:choose>
          </xsl:if>
        </xsl:if>
