...
def gen_import_file(db):
    ln = [
        ...
        '# -- Variables --', 'user=' + users[db], 'password=' + passwords[db],
        'host=localhost', 'schema=' + schemas[db], 'db_name=' + db_names[db],
        'sql_bin=' + sql_bin[db], 'import_bin=' + import_bins[db],
        'import_order_file=' + import_order_file, 'data_path=' + data_path,
        'reset_file=' + reset_files[db], 'ddl_file=' + ddl_files[db] + '\n',
        '# -- Code --', reset_before_statements[db],
        create_schema_statements[db], 'while IFS= read -r table', 'do',
        import_statements[db], 'done < "$import_order_file"'
    ]
